...
export function CraftingPanel () {
  ...
  const checkRequiredResources = useGameStore(
    state => state.checkRequiredResources
  )
  ...
  const displayedItems = inventoryPool.filter(
          ...
          {displayedItems.map(item => {
            ...
            return (
              <ToolTip
                ...
                disabled={!canCraft}
              >
                <div id={`craft-${item.id}`} className={`relative`}>
                  <ListItem
                    unlockable={true}
                    ...
                    amount={
                      item.requirements
                        .map(
                          req =>
                            `${req.quantity}x ${
                              resourcePool.find(res => res.id === req.id)
                                ?.name || 'Unknown'
                            }`
                        )
