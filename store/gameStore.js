...
const useGameStore = create((set, get) => ({
  ...
  resetRecentlyUpdatedInventoryItem: itemId =>
    set(() => ({ recentlyUpdatedInventoryItem: itemId })),
  setRecentlyUpdatedResource: resourceId =>
    set(() => ({ recentlyUpdatedResource: resourceId })),
  ...
  transmute: () => {
    const {
      ...
      maxEnergy,
      energy,
      xpFromCraftingMultiplier,
      xpFromSalesMultiplier,
      defaultSalesMultiplier
    } = get()

    // Core factors for randomness
    const baseValue = level * 2 // Base value scales with the level
    const energyFactor = energy / maxEnergy // Factor depends on current energy
    const multiplierFactor =
      (xpFromCraftingMultiplier +
        xpFromSalesMultiplier +
        defaultSalesMultiplier) /
      3 // Average multipliers

    // Random range influenced by stats
    const minRange = baseValue * 0.1 * multiplierFactor * energyFactor
    const maxRange = baseValue * 1.2 * multiplierFactor * energyFactor

    // Generate random value within the range
    const calculated = Math.random() * (maxRange - minRange) + minRange

    const { gainExperience } = get()
    gainExperience(calculated.toFixed(2))
  },
    ...
    set(state => {
      ...
      if (levelChanged) {
        ...
        setTimeout(() => {
          useDebugStore
            .getState()
            .setDebugMessage(`Leveled up! Now level ${newLevel}`, 'blue')
        }, 0)
      }
  ...
  setXpFromSalesMultiplier: multiplier =>
    set(() => ({
      xpFromSalesMultiplier: multiplier
    })),

  setXpFromCraftingMultiplier: multiplier =>
    set(() => ({
  ...
  applyUpgrade: upgradeId =>
    set(state => {
      ...
      const updatedUpgradesPool = state.upgradesPool.filter(
        u => u.id !== upgradeId
      )
      let newState = {
        ...state,
      ...
      if (upgrade.effect?.modifyMaxEnergy) {
        newState = {
          ...newState,
          ...handleMaxEnergyUpgrade(newState, upgrade.effect.modifyMaxEnergy)
        }
      }
      if (upgrade.effect?.increaseEnergyGain) {
        newState = {
          ...newState,
          ...handleIncreaseEnergyGainUpgrade(
            newState,
            upgrade.effect.increaseEnergyGain
          )
        }
      }
      ...
      if (upgrade.effect?.resourceBonus) {
        ...
        newState = {
          ...newState,
          ...handleResourceBonusUpgrade(newState, resource, amount)
        }
      }
      if (upgrade.effect?.incomeRateIncrease) {
        newState = {
          ...newState,
          ...handleIncomeRateIncrease(
            newState,
            upgrade.effect.incomeRateIncrease
          )
        }
      }
      useDebugStore
        .getState()
        .setDebugMessage(`Applied upgrade: ${upgrade.name}`, 'blue')
      return newState
    }),
  ...
  startAutoGather: (resourceId, rate) => {
    ...
    const timer = setInterval(() => {
      ...
      if (fractionalGather[resourceId] >= 1) {
        ...
        const resourceItem = updatedResources.find(
  ...
  stopAutoGather: resourceId => {
    ...
    if (state.gatherTimers[resourceId]) {
      ...
      set({
        gatherTimers: updatedTimers,
        fractionalGather: updatedFractionalGather
      })
    }
  },
  ...
  gatherResource: resourceId =>
    set(state => {
      ...
      if (!resource) {
        useDebugStore
          .getState()
          .setDebugMessage(`Failed to gather: ${resourceId} not found`, 'red')
      ...
      if (state.energy < resource.energyCost) {
        useDebugStore
          .getState()
          .setDebugMessage(
            `Insufficient energy for gathering resource: ${resourceId}`,
            'red'
          )
      ...
      useDebugStore
        .getState()
        .setDebugMessage(
          `Gathered ${totalYield} ${resource.name} and gained ${resource.xpGain} XP`,
          'green'
        )
      return {
  ...
  craftItem: itemId =>
    set(state => {
      ...
      const hasRequiredResources = item.requirements.every(req => {
        ...
        if (!resource || resource.quantity < req.quantity) {
          useDebugStore
            .getState()
            .setDebugMessage(
              `Missing resource: ${req.id} or insufficient quantity.`,
              'red'
            )
        }
      ...
      if (!hasRequiredResources) {
        useDebugStore
          .getState()
          .setDebugMessage(`Not enough resources to craft ${itemId}`, 'red')
        return state
      }
  ...
  sellItem: itemId =>
    set(state => {
      ...
      const inventoryItem = state.inventoryPool.find(
        poolItem => poolItem.id === itemId
      )
      ...
      useDebugStore
        .getState()
        .setDebugMessage(`Sold a ${itemId} for ${itemCost}`, 'green')

      // Gain experience based on the item's cost and multiplier
      useGameStore
        .getState()
        .gainExperience(
          itemCost * defaultSalesMultiplier * xpFromSalesMultiplier
        )
